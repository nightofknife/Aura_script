...
class OcrService:
        ...
        async with self._engine_lock:
            if self._engine is None:
                try:
                    ...
                    raise RuntimeError(
                        "OcrService requires optional dependency 'paddleocr'. "
                        "Install runtime dependencies with `pip install -r requirements/runtime.txt`."
                    ) from exc
                ...
                local_model_dirs = self._local_model_dirs()
                if local_model_dirs:
                    ocr_kwargs.update(local_model_dirs)
                else:
                    logger.info(
                        "OCR service: local model directories are not bundled; "
                        "PaddleOCR will resolve/download models using its own cache."
                    )
                try:
    ...
    def _local_model_dirs() -> dict[str, str]:
        model_root = Path(__file__).resolve().parent / "ocr_model"
        candidates = {
            "doc_orientation_classify_model_dir": model_root / "PP-LCNet_x1_0_doc_ori",
            "text_detection_model_dir": model_root / "PP-OCRv5_server_det",
            "text_recognition_model_dir": model_root / "PP-OCRv5_server_rec",
            "textline_orientation_model_dir": model_root / "PP-LCNet_x1_0_textline_ori",
        }
        existing = {
            key: str(path)
        ...
        return existing if len(existing) == len(candidates) else {}
