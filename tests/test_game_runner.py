...
class TestGameRunners(unittest.TestCase):
    ...
    def test_embedded_runner_target_status_uses_runtime_target_service(self):
        runner = EmbeddedGameRunner()
        fake_service = Mock()
        fake_service.target_summary.return_value = {"title": "Sample Game", "client_rect_screen": [1, 2, 3, 4]}
        with (
            ...
            result = runner.target_status(game_name="sample_game")

        self.assertTrue(result["ok"])
        self.assertEqual(result["game_name"], "sample_game")
        self.assertEqual(result["target"]["title"], "Sample Game")
    ...
    def test_embedded_runner_target_snapshot_serializes_runtime_capture(self):
        ...
        fake_service.target_summary.return_value = {"title": "Sample Game"}
        with (
            patch.object(runner, "_ensure_running_runtime", return_value=object()),
            patch("packages.aura_game.runner.service_registry.get_service_instance", return_value=fake_service),
        ):
            result = runner.target_snapshot(game_name="sample_game", backend="gdi")
    ...
    def test_subprocess_runner_target_helpers_use_request_channel(self):
        ...
        with patch.object(runner, "_request", return_value={"ok": True}) as request:
            result = runner.target_status(game_name="sample_game")
            self.assertTrue(result["ok"])
            request.assert_called_once_with("target_status", game_name="sample_game")

        with patch.object(runner, "_request", return_value={"ok": True}) as request:
            result = runner.target_snapshot(game_name="sample_game", backend="gdi")
            self.assertTrue(result["ok"])
            request.assert_called_once_with("target_snapshot", game_name="sample_game", backend="gdi")
