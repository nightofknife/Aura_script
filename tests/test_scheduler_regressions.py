...
def test_observability_metrics_update_is_rate_limited(monkeypatch, tmp_path):
    ...
    asyncio.run(
        service.ingest_event(
            Event(name="queue.enqueued", payload={"cid": "cid-1", "game_name": "sample_game", "task_name": "tasks:demo"})
        )
    )
    asyncio.run(
        service.ingest_event(
            Event(name="queue.dequeued", payload={"cid": "cid-1", "game_name": "sample_game", "task_name": "tasks:demo"})
        )
    )
