...
本文档记录 Aura Windows x64 运行包的本地打包和 GitHub Actions 自动发布流程。CPU 与 GPU+CPU 主包都包含 `AuraYihuanGui.exe`，用户解压后可以双击启动异环 GUI。
...
```text
CPU-only: 下载 aura-release-cpu-onnx.zip，解压后双击 AuraYihuanGui.exe 启动 GUI
GPU+CPU: 下载 aura-release-gpu-onnx.zip，解压后双击 AuraYihuanGui.exe 启动 GUI
...
NVIDIA overlay 的 zip 根目录与 GPU 主包保持同名顶层目录。最简单的用户流程是：把 `aura-release-gpu-onnx.zip` 和 `aura-release-gpu-onnx-nvidia-overlay.zip` 放在同一个文件夹，两个都解压到当前文件夹；如果系统提示合并或覆盖，选择允许。最终仍然只进入 `aura-release-gpu-onnx` 文件夹并双击 `AuraYihuanGui.exe`。

CPU 主包解压后的关键结构：

```text
aura-release-cpu-onnx/
  AuraYihuanGui.exe
  run.ps1
  config.yaml
  BUILD-INFO.txt
  runtime/
    aura.exe
    AuraYihuanRuntime.exe
    _internal/
      PySide6/
      onnxruntime/
  plans/
  models/ocr/ppocrv5_server/
```

GPU 主包解压后的关键结构：

```text
aura-release-gpu-onnx/
  AuraYihuanGui.exe
  run.ps1
  config.yaml
  BUILD-INFO.txt
...
```text
build-cpu: 构建带 AuraYihuanGui.exe 的 aura-release-cpu-onnx.zip
build-gpu: 构建带 AuraYihuanGui.exe 的 aura-release-gpu-onnx.zip
...
BUILD-INFO.txt 中 gui_entrypoint=AuraYihuanGui.exe
```
...
CPU 和 GPU 主包还要跑 GUI runtime self-check：
